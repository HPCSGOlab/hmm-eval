...
# - 1.6.0-nv7 : Fixed --tmpdir behavior when using a fallback decompressor
#
...
cat_decompress="tail -n +\${skip_decompress} \$0 | head -n \${size_decompress}"

if [ -z "$EMBED_DECOMPRESS" ]; then
    cat_decompress="echo \"No decompressor embedded in this archive\" >&2; exit 1"
fi
...
# The embedded decompressor, if present, needs to be extracted if a suitable
# decompressor is not already present. This needs to be done before extracting
# or listing the archive contents.
...
if [ -n "$EMBED_DECOMPRESS" ]; then
	...
	if ! command -v "$COMPRESS" > /dev/null 2>&1; then
	    decompressDir=\`mktemp -d "\$TMPROOT/makeself.XXXXXXXX" 2> /dev/null\`/
...
cat <<-EOF >> $archname
	...
	UnTAR() {
	    tar xvf - 2> /dev/null || {
	        echo "Extraction failed." >&2; kill -15 \$$;
	    };
	}
	...
	[ "\$keep" = "y" ] || trap '\$echo "Signal caught, cleaning up" >&2; cd \$TMPROOT; rm -rf \$tmpdir; exit 15' 1 2 15
