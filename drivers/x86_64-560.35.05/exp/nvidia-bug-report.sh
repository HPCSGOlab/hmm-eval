...
NVIDIA_BUG_REPORT_CHANGE='$Change: 34685233 $'
...
dump_mlx() {
    ...
    MLX_CSV="$tmp_dir/mlx$$.csv"
    MLX_INFO="$tmp_dir/mlx$$.info"
    ...
    for port in $(seq 1 5); do
        echo "$1 -d $2 -p $port --amber_collect $MLX_CSV > $MLX_INFO 2>&1"
        "$1" -d "$2" -p $port --amber_collect "$MLX_CSV" > "$MLX_INFO" 2>&1
        if [ $? -eq 0 ]; then
            ...
            echo "failed with error $? retrying..."
            cat "$MLX_INFO" | sed -Ee 's/\x1b\[[0-9;]+m//g' 2> /dev/null
            rm -f "$MLX_INFO" 2> /dev/null
            rm -f "$MLX_CSV" 2> /dev/null
            echo "____________________________________________"
...
(
    ...
    if [ $? -eq 0 -a -x "$mst" ]; then
        ...
        if [ $? -eq 0 -a -x "$mlxlink" ]; then
            ...
            echo "____________________________________________"
            echo ""
            report_command "$mst start"
            echo "____________________________________________"
            echo ""

            # scans and adds gpu devices
            report_command "$mst gpu add"
            echo "____________________________________________"
            echo ""

            # scans and add ib devices
            report_command "$mst ib add mlx5_0"
            echo "____________________________________________"
            echo ""
            ...
            "$mst" status -v |
            {
                ...
                if [ $? -eq 0 ]; then
                    ...
                    if [ ! -z "$resourcedump" ]; then
                        report_command "$resourcedump dump -d $devName -s 0x5025"
                        echo "____________________________________________"
                        echo ""
                    fi
                fi
                echo $tmp_line | grep "Quantum" | grep "mlx5_0" > /dev/null 2>&1
                if [ $? -eq 0 ]; then
                ...
                echo $tmp_line | grep "ConnectX" | grep "pciconf0.[2,3]" > /dev/null 2>&1
                if [ $? -eq 0 ]; then
                    echo "Starting ConnectX dump..."$tmp_line
                    devName=`echo $tmp_line | awk -F" " 'NR == 1 {print $2}';`
                    dump_mlx "$mlxlink" "$devName"
                    if [ ! -z "$mstdump" ]; then
